...
const errorHandler = (error, req, res, next) => {
  ...
  switch (error.code) {
    case EErrors.INVALID_TYPE_ERROR:
      return res.status(400).json({ success: false, error: `${error.message} - ${error.cause}` });

    case EErrors.NOT_FOUND_ERROR:
      return res.status(404).json({ success: false, error: `${error.message} - ${error.cause}` });
    ...
    case EErrors.DATABASE_ERROR:
      return res.status(500).json({ success: false, error: `${error.message} - ${error.cause}` });
